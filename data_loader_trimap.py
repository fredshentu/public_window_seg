...
def crop_data_noop(data, xc, yc, side, sawyer_data = False):
    data = tf.image.crop_to_bounding_box(data, tf.cast(yc-side/2.0, tf.int32), tf.cast(xc-side/2.0, tf.int32), \
                                                            tf.cast(side, tf.int32) , tf.cast(side, tf.int32))
    data = tf.image.random_flip_up_down(data)
    data = tf.image.random_flip_left_right(data)
    image = tf.transpose(tf.gather(tf.transpose(data, [2,0,1]), [0,1,2]), [1,2,0])
    mask = tf.transpose(tf.gather(tf.transpose(data, [2,0,1]), [3]), [1,2,0])
    noop = tf.transpose(tf.gather(tf.transpose(data, [2,0,1]), [4]), [1,2,0])

    image = tf.image.resize_images(image, [inpSize, inpSize])
    mask = tf.image.resize_images(mask, [maskSize, maskSize])
    noop = tf.image.resize_images(noop, [maskSize, maskSize])

    if not sawyer_data:
        mask = (mask + 1) / 2
    mask = tf.cast(tf.transpose(mask, [2,0,1])[0], tf.int32) #msk rank2
    noop = tf.cast(tf.transpose(noop, [2,0,1])[0], tf.int32) #msk rank2

    return image, mask, noop
...
def inputs_poking(filenames,
    ...
    with tf.name_scope('input'):
        ...
        noop = tf.convert_to_tensor(np.zeros([batch_size, maskSize, maskSize], dtype=tf.int32))
        
        #if not addBg, background = [-1.0, -1.0, -1.0 ......]
        return image, mask, score, background, noop
...
def read_decode_negative_sawyer_data(filename_queue,\
    ...
    if addBg:
        ...
        background = tf.image.resize_images(background, [bgSize, bgSize])
        return img, -1.0, 0, background, 0
    else:
        return img, -1.0, 0, -1.0, 0
...
def read_decode_positive_sawyer_data(filename_queue, pos_shift,\
    ...
    features = tf.parse_single_example(
          ...
          features={
                  ...
                  'noop':tf.FixedLenFeature([], tf.string),
                  'background': tf.FixedLenFeature([], tf.string),
          })
    img = tf.decode_raw(features['img'], tf.uint8)
    img = tf.reshape(img, [448,448,3])
    ...
    noop = tf.decode_raw(features['noop'],tf.uint8)
    noop = tf.cast(noop, tf.float32)
    noop = tf.reshape(noop, [448,448,1])
    
    img_mask_noop = tf.concat([img, mask, noop],2)
    img_mask_noop = tf.pad(img_mask_noop, [[padSize,padSize],[padSize,padSize],[0,0]])
    ...
    img, mask, noop = crop_data_noop(img_mask_noop, xc, yc, side, sawyer_data = True)
    if addBg:
        background = tf.decode_raw(features['background'], tf.uint8)
        background = tf.reshape(background, [160,160,3])
        background = tf.cast(background, tf.float32)
        background = (background/255.0) - 0.5
        background = tf.image.resize_images(background, [bgSize, bgSize])
        return img, mask, 1, background, noop
    else:
        return img, mask, 1, -1.0, noop
...
def inputs_sawyer_data(filenames, mode, pos_max, neg_min, jetter_max=90, train=True, \
    ...
    with tf.name_scope('input'):
        ...
        if mode == "positive":
            image, mask, score, background, noop = read_decode_positive_sawyer_data(filename_queue,pos_max, addBg)
        
        elif mode == "negative":
            image, mask, score, background, noop = read_decode_negative_sawyer_data(filename_queue,addBg)
        elif mode == "negative_from_positive":
            image, mask, score, background, noop = read_decode_negative_from_positive_sawyer_data(filename_queue,\
