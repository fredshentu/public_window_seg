...
from data_loader_trimap import inputs_poking, inputs_sawyer_data
slim = tf.contrib.slim


def gen_name(prefix, ratio, pos_max, neg_min, trunk, decay, lr_factor, background):
    return '%s_%s_lambda_%.2f_a_%d_b_%d_decay_%.5f_lr_factor%.2f_background_%s_trimap' % (prefix, \
...
def main():
    ...
    train_pos_segment_img_poke, train_pos_segment_mask_poke, train_pos_segment_score_poke, train_pos_segment_background_poke, train_pos_segment_noop_poke = inputs_poking(train_set_old_names,\
                                        pos_max=args.pos_max, neg_min=args.neg_min, positive=True, addBg=args.add_background, batch_size=8)

    train_pos_scoring_img_poke, train_pos_scoring_mask_poke, train_pos_scoring_score_poke, train_pos_scoring_background_poke, train_pos_scoring_noop_poke = inputs_poking(train_set_old_names, \
                                        pos_max=0.1, neg_min=args.neg_min, positive=True, addBg=args.add_background, batch_size=4)

    train_neg_img_poke, train_neg_mask_poke, train_neg_score_poke, train_neg_background_poke, train_neg_noop_poke = inputs_poking(train_set_old_names,\
                                        pos_max=args.pos_max, neg_min=args.neg_min, positive=False, addBg=args.add_background, batch_size=4)
    
    
    train_pos_segment_img_sawyer, train_pos_segment_mask_sawyer, train_pos_segment_score_sawyer, train_pos_segment_background_sawyer, train_pos_segment_noop_sawyer = \
                                    inputs_sawyer_data(train_set_new_pos_names, "positive", args.pos_max, \
                                                                    args.neg_min, batch_size = 24, addBg=args.add_background)
    train_pos_scoring_img_sawyer, train_pos_scoring_mask_sawyer, train_pos_scoring_score_sawyer, train_pos_scoring_background_sawyer, train_pos_scoring_noop_sawyer = \
                                    inputs_sawyer_data(train_set_new_pos_names, "positive", 0.1, \
                                                                    args.neg_min, batch_size = 12, addBg=args.add_background)

    train_neg_img_sawyer, train_neg_mask_sawyer, train_neg_score_sawyer, train_neg_background_sawyer, train_neg_noop_sawyer = \
                                    inputs_sawyer_data(train_set_new_neg_names, "negative", args.pos_max, \
                                                                    args.neg_min, batch_size = 6, addBg=args.add_background)
    train_neg_from_pos_img_sawyer, train_neg_from_pos_mask_sawyer, train_neg_from_pos_score_sawyer, train_neg_from_pos_background_sawyer, train_neg_from_pos_noop_sawyer = \
    ...
    train_segment_noop = tf.concat([
        train_pos_segment_noop_sawyer,
        train_pos_segment_noop_poke,
    ])
    ...
    train_mask_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=train_pred_mask, labels=train_segment_mask) * (1. - train_segment_noop)
    ...
    model_name = gen_name('train_sgd', args.mask_ratio, args.pos_max, args.neg_min, args.trunk, args.weight_decay, args.lr_factor, args.add_background)
