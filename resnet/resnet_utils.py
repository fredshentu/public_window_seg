...
def conv2d_same(inputs, num_outputs, kernel_size, stride, rate=1, scope=None, reuse=False):
  ...
  #                   [[0, 0], [pad_beg, pad_end], [pad_beg, pad_end], [0, 0]], mode='SYMMETRIC')
  #   return slim.conv2d(inputs, num_outputs, kernel_size, stride=stride,
  #                      rate=rate, padding='VALID', scope=scope)


  # in_height, in_width = inputs.get_shape().as_list()[1:3]
  # out_height = np.ceil(float(in_height) / float(stride))
  # out_width  = np.ceil(float(in_width) / float(stride))
  # pad_along_height = ((out_height - 1) * stride +
  #                   kernel_size - in_height)
  # pad_along_width = ((out_width - 1) * stride +
  #                    kernel_size - in_width)
