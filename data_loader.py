...
def read_decode_positive_example_poking(filename_queue, shift=24, pad_size=100, with_background=False):
...
def read_decode_negative_example_poking(filename_queue, neg_shift_min=46, neg_shift_max=64, pad_size=200, with_background=False):
    reader = tf.TFRecordReader()
    _, serialized_example = reader.read(filename_queue)
    features = tf.parse_single_example(
          serialized_example,
