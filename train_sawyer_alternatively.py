...
def main():
    ...
    train_msk_loss = args.mask_ratio * train_mask_loss + args.weight_decay * train_decay_loss
    train_label_loss = train_label_loss + args.weight_decay * train_decay_loss
    ...
    train_opt_msk = slim.learning.create_train_op(train_msk_loss, optimizer, clip_gradient_norm=40.0)
    train_opt_label = slim.learning.create_train_op(train_label_loss, optimizer, clip_gradient_norm=40.0)
    
    sess.run(tf.initialize_variables(set(tf.all_variables()) - tmp_vars))

    model_name = gen_name('debug_train_sgd_alter', args.mask_ratio, args.pos_max, args.neg_min, \
    ...
    for timestep in range(args.num_itr):
        ...
        if np.random.rand() < 0.5:
            train_opt = train_opt_msk
        else:
            train_opt = train_opt_label
