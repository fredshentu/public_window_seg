...
def main():
    ...
    val_segment_img   = val_pos_segment_img
    ...
    if args.trunk == 'vgg':
        ...
        val_pred_mask, _  = build_resnet50_network(val_segment_img, background=val_segment_background, sess=sess, reuse=True, is_training=False, dropout=1.0, add_background=args.add_background)
        ...
        _, val_pred_score = build_resnet18_network(val_scoring_img, background=val_scoring_background, sess=sess, reuse=True, is_training=True, dropout=1.0, add_background=args.add_background)
        val_pred_mask, _  = build_resnet18_network(val_segment_img, background=val_segment_background, sess=sess, reuse=True, is_training=True, dropout=1.0, add_background=args.add_background)
    ...
    val_total_loss = val_mask_loss * args.mask_ratio + val_label_loss + args.weight_decay * decay_loss
    ...
    img_summ = []
    train_pos_img_r, train_pos_img_g, train_pos_img_b = tf.unstack(tf.image.resize_images((train_pos_segment_img+0.5)*255.0, [112, 112]), axis=-1)
    # import pdb; pdb.set_trace()
    train_pos_img_viz_g = (1. - tf.cast(train_pos_segment_mask, tf.float32)) * train_pos_img_g
    train_pos_img_viz_b = (1. - tf.cast(train_pos_segment_mask, tf.float32)) * train_pos_img_b
    ...
    img_summ.append(tf.summary.image('train/neg_img', train_neg_img, max_outputs=10))

    val_pos_img_r, val_pos_img_g, val_pos_img_b = tf.unstack(tf.image.resize_images((val_pos_segment_img+0.5)*255.0, [112, 112]), axis=-1)
    # import pdb; pdb.set_trace()
    val_pos_img_viz_g = (1. - tf.cast(val_pos_segment_mask, tf.float32)) * val_pos_img_g
    val_pos_img_viz_b = (1. - tf.cast(val_pos_segment_mask, tf.float32)) * val_pos_img_b
    val_pos_img_viz = tf.stack([val_pos_img_r, val_pos_img_viz_g, val_pos_img_viz_b], axis=-1)

    # import pdb; pdb.set_trace()
    val_pos_img_pred_g = tf.unstack(tf.nn.softmax(tf.stack(tf.unstack(val_pred_mask, axis=0), axis=0), dim=-1),axis=-1)[0] * val_pos_img_g
    val_pos_img_pred_b = tf.unstack(tf.nn.softmax(tf.stack(tf.unstack(val_pred_mask, axis=0), axis=0), dim=-1),axis=-1)[0] * val_pos_img_b
    val_pos_img_pred_viz = tf.stack([val_pos_img_r, val_pos_img_pred_g, val_pos_img_pred_b], axis=-1)


    img_summ.append(tf.summary.image('val/pos_img', val_pos_img_viz, max_outputs=10))
    img_summ.append(tf.summary.image('val/pos_img_pred', val_pos_img_pred_viz, max_outputs=10))
    img_summ.append(tf.summary.image('val/neg_img', val_neg_img, max_outputs=10))
